...
void ssdr::match_samples_to_curve2(void)
{
    rp_sample_curves = new int[rest_pose_samples.rows()];
    ...
    for(int i = 0; i < rest_pose_samples.rows(); i++)
    {
        Point2 arbPoint = { rest_pose_samples(i,0), rest_pose_samples(i,1) }; // the sample point

        for(int j = 0; j < rp_curves.size(); j++)
        {
            int c_i = (rp_curves.at(j))(0);
            int c_i2 = (rp_curves.at(j))(3);
            int d_i = (rp_curves.at(j))(1);
            int d_i2 = (rp_curves.at(j))(2);

            //  the cubic Bezier curve
            Point2 bezCurve[4] = {	
                { rest_pose(c_i,0), rest_pose(c_i,1) },
                { rp_tangents(d_i,0), rp_tangents(d_i,1) },
                { rp_tangents(d_i2,0), rp_tangents(d_i2,1) },
                { rest_pose(c_i2,0), rest_pose(c_i2,1) },
                };
            ...
            // find the distance of the sample point to this point if the distance
            ...
            if( dista <= 1 )
            {
            printf("\ndist between points = %lf\n\n", dista);
                // record its curve number and its parameter t
                rp_samples_t.push_back(t_param);
                rp_sample_curves[i] = j;
                // set its tangent
                rp_tg_samples(i,0) = 3*(1-t_param)*(1-t_param)*(bezCurve[1].x-bezCurve[0].x);
                rp_tg_samples(i,0) += 6*(1-t_param)*t_param*(bezCurve[2].x-bezCurve[1].x);
                rp_tg_samples(i,0) += 3*t_param*t_param*(bezCurve[3].x-bezCurve[2].x);
                
                rp_tg_samples(i,1) = 3*(1-t_param)*(1-t_param)*(bezCurve[1].y-bezCurve[0].y);
                rp_tg_samples(i,1) += 6*(1-t_param)*t_param*(bezCurve[2].y-bezCurve[1].y);
                rp_tg_samples(i,1) += 3*t_param*t_param*(bezCurve[3].y-bezCurve[2].y);

                printf(" rp_tg_samples(i,-) == %lf    %lf\n",rp_tg_samples(i,0),rp_tg_samples(i,1));
    ...
    dp_sample_curves = new int[deformed_pose_samples.rows()];
    Eigen::MatrixXd deformed_pose = frame_poses.at(0);
    dp_tg_samples = Eigen::MatrixXd::Zero(deformed_pose_samples.rows(),2);

    for(int i = 0; i < deformed_pose_samples.rows(); i++)
    {
        Point2 arbPoint = { deformed_pose_samples(i,0), deformed_pose_samples(i,1) }; // the sample point

        for(int j = 0; j < df_curves.size(); j++)
        {
            int c_i = (df_curves.at(j))(0);
            int c_i2 = (df_curves.at(j))(3);
            int d_i = (df_curves.at(j))(1);
            int d_i2 = (df_curves.at(j))(2);

            //  the cubic Bezier curve
            Point2 bezCurve[4] = {	
                { deformed_pose(c_i,0), deformed_pose(c_i,1) },
                { df_tangents(d_i,0), df_tangents(d_i,1) },
                { df_tangents(d_i2,0), df_tangents(d_i2,1) },
                { deformed_pose(c_i2,0), deformed_pose(c_i2,1) },
                };
            ...
            {
                // record its curve number and its parameter t
                dp_samples_t.push_back(t_param);
                dp_sample_curves[i] = j;
                // set its tangent
                dp_tg_samples(i,0) = 3*(1-t_param)*(1-t_param)*(bezCurve[1].x-bezCurve[0].x);
                dp_tg_samples(i,0) += 6*(1-t_param)*t_param*(bezCurve[2].x-bezCurve[1].x);
                dp_tg_samples(i,0) += 3*t_param*t_param*(bezCurve[3].x-bezCurve[2].x);
                
                dp_tg_samples(i,1) = 3*(1-t_param)*(1-t_param)*(bezCurve[1].y-bezCurve[0].y);
                dp_tg_samples(i,1) += 6*(1-t_param)*t_param*(bezCurve[2].y-bezCurve[1].y);
                dp_tg_samples(i,1) += 3*t_param*t_param*(bezCurve[3].y-bezCurve[2].y);
...
//            printf(" %d ",smooth[l1+l2*num_labels]);
 ...
 //   printf(" %d ",smooth[14+17*num_labels]);
    ...
    try{
        ...
        set_neighborhood2(numNeighbors,neighborsIndexes,neighborsWeights);
...
//            printf(" for point %d label %d was chosen\n",i,result[i]);
    ...
    // for each point we now apply the label and find the closest point
    //      we consider the closest point the corresponding deformed point
    // we then recompute R and T s which now we will have only n labels
    
    std::vector<int> correspondance_index(rest_pose_samples.rows());


    for( int i1 = 0; i1 <rest_pose_samples.rows(); i1++)
    {
        // look up the label assigned to this vertex
        int label_number = result[i1];
        int mod_n = rest_pose_samples.rows();
        // get the rotation and translation of this label
        Eigen::MatrixXd label_R = pair_R[label_number / mod_n][label_number % mod_n];
        Eigen::Vector2d label_T = pair_T[label_number / mod_n][label_number % mod_n];
        std::cout << label_R << std::endl;
        std::cout << label_T << std::endl;

        // get the rest pose coordinates
        Eigen::Vector2d point1(0,0);
        point1(0) = rest_pose_samples(i1,0);
        point1(1) = rest_pose_samples(i1,1);
        
        // calculate the transformed rest pose
        point1 = label_R * point1 + label_T;
        
        double min_d1 = 0;
        // go through all the deformed pose coordinates and find out which point is
        // closest to this point
        for( int j1 = 0; j1 < deformed_pose_samples.rows(); j1++)
        {
            // get the deformed pose coordinates
            Eigen::Vector2d point2(0,0);
            point2(0) = deformed_pose_samples(j1,0);
            point2(1) = deformed_pose_samples(j1,1);
            
            //calculate eucledean distance
            point2 = point2 - point1;
            double ed = point2.norm();
            if( j1 == 0 )
            {
                min_d1 = ed; 
                correspondance_index[i1] = 0;
            }
            else if( min_d1 > ed )
            {
                min_d1 = ed; 
                correspondance_index[i1] = j1;
            }
        }

    }
    
    std::vector<Eigen::Matrix2d> R_labels;
    std::vector<Eigen::Vector2d> T_labels;
    int loop1_itr = 0;
...
loop1:
    ...
    // now having the corresponding point we can recompute R and T s
    for( int i1 = 0; i1 <rest_pose_samples.rows(); i1++)
    {
        // look up the label assigned to this vertex
        int label_number = result[i1];
        std::vector<int> p_in_cluster;
        std::vector<int> df_cluster;
        std::vector<int> df_cluster_unique;
        Eigen::Vector2d p_bar(0, 0);
        Eigen::Vector2d q_bar(0, 0);


        // now find the center of the cluster from all points having the same label
        for( int i2 = 0; i2 < rest_pose_samples.rows(); i2++)
        {
            if( result[i2] == label_number)
            {
                p_in_cluster.push_back(i2);

                p_bar(0) += rest_pose_samples(i2,0);
                p_bar(1) += rest_pose_samples(i2,1);
                
                // ?? here we assume that the corresponding indexes are different for each rest point??
                // but this might not be the case.
                int def_ind = correspondance_index[i2];
                df_cluster.push_back(def_ind);
                df_cluster_unique.push_back(def_ind);


            }
        }
        p_bar/=(int)p_in_cluster.size();

        std::cout << " P bar value " << p_bar << std::endl;
        
        //getting rid of duplicate indexes to calculate the center of the deformed cluster correctly
        sort( df_cluster_unique.begin(), df_cluster_unique.end() );
        df_cluster_unique.erase( unique( df_cluster_unique.begin(), df_cluster_unique.end() ), df_cluster_unique.end());

        for( auto i3 : df_cluster_unique )
        {
            q_bar(0) += deformed_pose_samples(i3,0);
            q_bar(1) += deformed_pose_samples(i3,1);
        }
        q_bar/=(int)df_cluster_unique.size();
        
        std::cout << " q bar value " << q_bar << std::endl;
        
        Eigen::MatrixXd X = Eigen::MatrixXd::Zero( 2, (int)p_in_cluster.size());
        Eigen::MatrixXd Y = Eigen::MatrixXd::Zero( 2, (int)p_in_cluster.size());
        ...
        int col_i = 0;
        for( int ll : p_in_cluster )
        {
            printf(" ll 1 = %d   ", ll );

            xi_x = rest_pose_samples(ll,0) - p_bar(0);
            xi_y = rest_pose_samples(ll,1) - p_bar(1);

            X.col(col_i) << xi_x, xi_y;
            col_i++;
        }
        col_i = 0;
        printf("\n");
        for( int ll : df_cluster )
        {

            printf(" ll 2 = %d   ", ll );

            yi_x = deformed_pose_samples(ll,0) - q_bar(0);
            yi_y = deformed_pose_samples(ll,1) - q_bar(1);

            Y.col(col_i) << yi_x, yi_y;
...
//        std::cout << " 2x2 covariance matrix" << std::endl << S_tmp << std::endl; 
  ...
  //              << std::endl << svd.matrixV() << std::endl;
        ...
        R_labels.push_back( R );

        // The optimal translation is q_bar - R p_bar
        T_labels.push_back( q_bar - (R*p_bar) );
    ...
    // now we should have less than n labels instead of n^2 labels
    // again we recompute the label assignment of each cluster

    // stores the number of labels
    num_labels = R_labels.size();
    printf(" numebr of labels = %d \n\n", num_labels);
    
    // setting up the array for data costs i.e. the cost of using Label l_i ( Ri,Ti ) for point p
    delete [] data;
    data = new int[num_samples * num_labels];
    
    // setting up the array for smooth costs
    delete [] smooth;
    smooth = new int[num_labels*num_labels];

    for( int i = 0; i <rest_pose_samples.rows(); i++)
    {
        int label_index = 0;
        printf("data terms for %d\n" , i );
        //for each sample in the rest_pose apply all possible R,T pairs one by one
        // to get the transformed point
        ...
        for( int ii = 0; ii < num_labels; ii++)
        {
            ...
            transformed_r_p = ( R_labels[ii] * rp_sample_p ) + T_labels[ii];
//            std::cout << " transformed rest sample point" << std::endl << transformed_r_p << std::endl;
            ...
            for(int kk = 0; kk < deformed_pose_samples.rows(); kk++)
            {
                ...
                {
                    min_label_cost = e_distance;
                    correspondance_index[ii] = 0;
                }
                // choose the vertex that gives the minimum
                else if( e_distance < min_label_cost )
                {
                    min_label_cost = e_distance;
                    correspondance_index[ii] = kk;
                }
            }

            data[i*num_labels + (ii) ] = (int)min_label_cost;
            printf(" %d  " , data[i*num_labels + (ii) ] );
...
{
    std::vector<std::vector<int>> nbh;

    for( int i = 0; i <rest_pose_samples.rows(); i++)
    {
        std::vector<int> temp;
        int cur_curve_num = rp_sample_curves[i];

        for( int j = 0; j < rest_pose_samples.rows(); j++)
        {
            if( (cur_curve_num - 1) == rp_sample_curves[j] )
            {
    ...
    {
        printf("%d === ",i);
        numNeighbors[i] = nbh[i].size();
        a[i] = new int[nbh[i].size()];
        neighborsWeights[i] = new int[nbh[i].size()];

        
        for(int k = 0 ; k < nbh[i].size(); k++)
        {
            a[i][k] = nbh[i].at(k);
                printf(" %d ", a[i][k]);
...
void ssdr::set_neighborhood2(int *numNeighbors,int **a,int** neighborsWeights)
{
    std::vector<std::vector<int>> nbh;
    ...
    for( int i = 0; i <rest_pose_samples.rows(); i++)
    {
        ...
        // indexes of the end points of both curves
        int c1_p0 = rp_curves[cur_curve_num](0);
        int c1_p3 = rp_curves[cur_curve_num](3);

        for( int j = 0; j < i; j++)
        {
            int cur_curve_num2 = rp_sample_curves[j];
            // if the curves are different compare their endpoints to see if they are neighboors
         //   if( cur_curve_num != cur_curve_num2 )
         //   {

                int c2_p0 = rp_curves[cur_curve_num2](0);
                int c2_p3 = rp_curves[cur_curve_num2](3);
                // find distances between each pair of end points 
                
                double distance = (rest_pose(c1_p0,0) - rest_pose(c2_p0,0)) * (rest_pose(c1_p0,0) - rest_pose(c2_p0,0));
                distance += (rest_pose(c1_p0,1) - rest_pose(c2_p0,1)) * (rest_pose(c1_p0,1) - rest_pose(c2_p0,1));
                ...
                distance = (rest_pose(c1_p0,0) - rest_pose(c2_p3,0)) * (rest_pose(c1_p0,0) - rest_pose(c2_p3,0));
                distance += (rest_pose(c1_p0,1) - rest_pose(c2_p3,1)) * (rest_pose(c1_p0,1) - rest_pose(c2_p3,1));
                ...
                distance = (rest_pose(c1_p3,0) - rest_pose(c2_p0,0)) * (rest_pose(c1_p3,0) - rest_pose(c2_p0,0));
                distance += (rest_pose(c1_p3,1) - rest_pose(c2_p0,1)) * (rest_pose(c1_p3,1) - rest_pose(c2_p0,1));
                ...
                distance = (rest_pose(c1_p3,0) - rest_pose(c2_p3,0)) * (rest_pose(c1_p3,0) - rest_pose(c2_p3,0));
                distance += (rest_pose(c1_p3,1) - rest_pose(c2_p3,1)) * (rest_pose(c1_p3,1) - rest_pose(c2_p3,1));
